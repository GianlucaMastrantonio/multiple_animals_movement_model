...
function sample_muC_lv2_type2!(rng::MersenneTwister,Likelihood::Vector{Likelihood_OU_CircLinmodel}, muCPar::VecParMvNormal, parHier::OptionHierarchicalParameters,Clusterization::Vector{Clusterization_HDPHMM})
    ...
    for ian in 1:nanim_v2
        ...
        for k1 in 1:Likelihood[ian].clusterization.n_nonemptyC[1]
            ...
            @inbounds k_beta  = parHier.h_eta.clust[k,ian]
            @inbounds pp      = parHier.h_eta.prob[k_beta]
            ...
            for j1 in 1:kmax
                @inbounds Aindex[k,(ian-1)*5+2] = j1
                if (parHier.h_eta.prob[j1]>u) & (check_unique(Aindex,k,kmax))

                    n_j         += one(n_j)
                    @inbounds indexj[n_j] = j1

                    @inbounds Likelihood[ian].eta.parameteracc[k] = deepcopy(parHier.h_eta.par.parameteracc[j1])

                    for t in t_index
                        @inbounds probvec[n_j] += compute_logpdf(Likelihood[ian], k, Int16(t+1))
                    end
                    #@inbounds probvec[n_j] += Float64(sum(Clusterization[ian].MatM[:,k])-1.0)*log(parHier.h_eta.prob[j1])

                    #print(probvec[n_j], " ", j1, " ", k ,"\n")
                end
            ...
            @inbounds Likelihood[ian].eta.parameteracc[k] = deepcopy(parHier.h_eta.par.parameteracc[samp_c])
            @inbounds parHier.h_eta.clust[k,ian]          = samp_c

            @inbounds Aindex[k,(ian-1)*5+2]                        = samp_c
        end
        ...
        if Likelihood[ian].clusterization.n_emptyC[1]>0
            for k1 in 1:Likelihood[ian].clusterization.n_emptyC[1]
                ...
                @inbounds probvec = zeros(Float64,kmax)
                @inbounds indexj  = zeros(Int16,kmax)
                n_j     = 0
                for j1 in 1:kmax
                    @inbounds Aindex[k,(ian-1)*5+2] = j1
                    if check_unique(Aindex,k,kmax)
                        n_j          += one(n_j)
                        @inbounds indexj[n_j]  = j1
                        @inbounds probvec[n_j] = parHier.h_eta.prob[j1]
                    end
                end
                @inbounds sampvec = probvec[1:n_j]
                @inbounds samp_c = indexj[sample_discretevar(rng,sampvec)]
...
function sample_muC_lv2!(rng::MersenneTwister,Likelihood::Vector{Likelihood_OU_CircLinmodel}, muCPar::VecParMvNormal, parHier::OptionHierarchicalParameters,Clusterization::Vector{Clusterization_HDPHMM})
...
function sample_muC_split_merge!(rng::MersenneTwister,Likelihood::Vector{Likelihood_OU_CircLinmodel}, muCPar::VecParMvNormal, parHier::OptionHierarchicalParameters,Clusterization::Vector{Clusterization_HDPHMM})
    ...
    for is in 1:nsamp
        ...
        ind_j1[is] = parHier.h_eta.clust[ind_k1[is],ind_a1[is]]
        ind_j2[is] = parHier.h_eta.clust[ind_k2[is],ind_a2[is]]
    ...
    for is in 1:nsamp
        ...
        if ind_j1[is]!=ind_j2[is]
            ...
            Likelihood[ind_a1[is]].eta.parameteracc[ind_k1[is]] = deepcopy(parHier.h_eta.par.parameteracc[ind_j2[is]])
            for t in t_index_1
                @inbounds prob1_new += compute_logpdf(Likelihood[ind_a1[is]], ind_k1[is], Int16(t+1))
            end

            Likelihood[ind_a2[is]].eta.parameteracc[ind_k2[is]] = deepcopy(parHier.h_eta.par.parameteracc[ind_j1[is]])
            for t in t_index_2
                @inbounds prob2_new += compute_logpdf(Likelihood[ind_a2[is]], ind_k2[is], Int16(t+1))
            end

            probvec[1] = prob1+prob2
            probvec[1] += Float64(n1)*log(parHier.h_eta.prob[ind_j1[is]])
            probvec[1] += Float64(n2)*log(parHier.h_eta.prob[ind_j2[is]])

            probvec[2] = prob1_new+prob2_new
            probvec[2] += Float64(n1)*log(parHier.h_eta.prob[ind_j2[is]])
            probvec[2] += Float64(n2)*log(parHier.h_eta.prob[ind_j1[is]])

            probvec[3] = prob1+prob2_new
            probvec[3] += Float64(n1)*log(parHier.h_eta.prob[ind_j1[is]])
            probvec[3] += Float64(n2)*log(parHier.h_eta.prob[ind_j1[is]])

            probvec[4] = prob1_new+prob2
            probvec[4] += Float64(n1)*log(parHier.h_eta.prob[ind_j2[is]])
            probvec[4] += Float64(n2)*log(parHier.h_eta.prob[ind_j2[is]])
            ...
            if samp_c==1
                ...
                Likelihood[ind_a1[is]].eta.parameteracc[ind_k1[is]] = deepcopy(parHier.h_eta.par.parameteracc[ind_j1[is]])
                parHier.h_eta.clust[ind_k1[is],ind_a1[is]]          = ind_j1[is]

                Likelihood[ind_a2[is]].eta.parameteracc[ind_k2[is]] = deepcopy(parHier.h_eta.par.parameteracc[ind_j2[is]])
                parHier.h_eta.clust[ind_k2[is],ind_a2[is]]          = ind_j2[is]

            elseif samp_c==2

                Likelihood[ind_a1[is]].eta.parameteracc[ind_k1[is]] = deepcopy(parHier.h_eta.par.parameteracc[ind_j2[is]])
                parHier.h_eta.clust[ind_k1[is],ind_a1[is]]          = ind_j2[is]

                Likelihood[ind_a2[is]].eta.parameteracc[ind_k2[is]] = deepcopy(parHier.h_eta.par.parameteracc[ind_j1[is]])
                parHier.h_eta.clust[ind_k2[is],ind_a2[is]]          = ind_j1[is]
